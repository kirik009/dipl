...
export default function UserManagement() {
  ...
  const [userToDelete, setUserToDelete] = useState<UserWithoutPassword | null>(
    null
  );
  const [userToEdit, setUserToEdit] = useState<UserWithoutPassword | null>(
    null
  );
  const [userToAddTask, setUserToAddTask] =
  ...
  const { data: assignedTasks, isLoading: assignedLoading } = useQuery<
    (AssingedTask & { taskName: string | null } & {
  ...
  const { data: assignedSolvedTasks, isLoading: assignedSolvedLoading } =
    useQuery<
      (AssingedTask & { taskName: string | null } & {
        authorName: string | null;
      })[]
  ...
  const { data: assignedExpiredTasks, isLoading: assignedExpiredLoading } =
    useQuery<
      (AssingedTask & { taskName: string | null } & {
        authorName: string | null;
      })[]
  ...
  const queries =
    assignedSolvedTasks?.map((task) => ({
      ...
      queryFn: async (): Promise<
        ({ exercisesNumber: number | null } & TaskProgress)[]
      > => {
        const res = await fetch(
          `/api/task_prog/${task.taskId}/${userToAddTask?.id}`
        );
  ...
  const handleAssign = (e: React.FormEvent) => {
    ...
    selectedExercisesToDelete.forEach(({ id }) => {
      deleteAssignedTaskMutation.mutate(id, {
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: [`/api/assignedTasks/${userToAddTask?.id}`],
          });
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    e.preventDefault();
    if (userToCreate) createUserMutation.mutate(userToCreate);
    setUserToCreate(null);
  };
  if (
    isLoading ||
    assignedSolvedLoading ||
    assignedExpiredLoading ||
    assignedLoading
  ) {
    return (
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          ...
          {!isMobile && user?.role === "admin" && (
            <Dialog
              open={!!userToCreate}
              onOpenChange={(open) => !open && setUserToCreate(null)}
            >
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                {assignedTasks && assignedTasks.length > 0 && (
                  <div className="space-y-2">
                    ...
                    {assignedTasks
                      ...
                      .map((task) => {
                        const selected = selectedExercisesToDelete.find(
                          (e) => e.id === task.id
                        );
                        return (
                          <>
                            ...
                            >
                              <div className="flex justify-between items-center">
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    if (e.target.checked) {
                                      setSelectedExercisesToDelete((prev) => [
                                        ...prev,
                                        { id: task.id },
                                      ]);
                                    } else {
                                      setSelectedExercisesToDelete((prev) =>
                                        prev.filter((ex) => ex.id !== task.id)
                                      );
                                    }
                                  }}
                                  className={cn(
                                              ...
                                              (taskProg, index) => (
                                                <div className="p-2 rounded w-64">
                                                  ...
                                                  <p>
                                                    ...
                                                    {taskProg.correctAnswers}/
                                                    {taskProg.exercisesNumber}
                                                  </p>
                                                </div>
                                              )
                                            )}
                ...
                {tasks
                  ?.filter(
                    (task) =>
                      !assignedTasks?.some(
                        (assigned) => assigned.taskId === task.id
                      )
                  )
                  .filter(
                    (task) =>
                      !assignedSolvedTasks?.some(
                        (assigned) => assigned.taskId === task.id
                      )
                  )
                  .filter(
                    (task) =>
                      !assignedExpiredTasks?.some(
                        (assigned) => assigned.taskId === task.id
                      )
                  )
                  .filter((task) =>
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedExercises((prev) => [
                                  ...prev,
                                  { exerciseId: task.id, dueDate: "" },
                                ]);
                              } else {
                                setSelectedExercises((prev) =>
                                  prev.filter((e) => e.exerciseId !== task.id)
                                );
                              }
                            }}
                          />
