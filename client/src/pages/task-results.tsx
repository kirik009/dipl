...
export default function TaskResults() {
  ...
  const { data: prog, isLoading: progLoading, error: progError } = useQuery<TaskProgress>({
        ...
        <div className="container mx-auto pt-20 pb-12 px-4 min-h-screen">
          <div className="max-w-md mx-auto bg-white p-8 rounded-xl shadow-sm mt-8 text-center">
            ...
            <p className="text-gray-600 mb-6">
              {error?.message || "Не удалось загрузить результаты. Попробуйте снова."}
            </p>
            <Button asChild>
    ...
    <>
      ...
      <div className="mt-16">
        <div className="flex flex-col items-center mb-6">
          <p className="mx-auto w-fit">Задание начато: {new Date(prog.startedAt).toLocaleString()}</p>
          <p className="mx-auto w-fit">
            Задание завершено:{" "}
            {prog.completedAt ? new Date(prog.completedAt).toLocaleString() : "—"}
          </p>
          <p className="mx-auto w-fit">
