...
export default function TasksPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery<(Task & { creatorFullName: string | null })[]>({
    queryKey: ["/api/tasks"],
    queryFn: async () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/task_exercises/${taskProg?.taskId}/seq/${nextSeq}`
      );
  ...
  const currentTasks =
    activeTab === "all" ? filteredAllTasks : filteredAssignedTasks;
  // Handle pagination
  ...
  if (taskProg?.isActive && nextExercise) {
    navigate(
      `/tasks/${taskProg.taskId}/prog/${taskProg.id}/exercises/${nextExercise.id}/seq/${nextSeq}`
    );
    toast({
      title: "Вы не доделали задание",
      description:
        "Пожалуйста, закончите это задание, чтобы приступить к другим упражнениям",
    });
    return null;
  }
  if (
                  ...
                  <td className="py-4 px-4 whitespace-nowrap text-sm text-gray-500 max-w-xs truncate">
                    {activeTab === "all" ? (
                      <a href={`/tasks/${task.id}`}>
                        {
                          (task as Task & { creatorFullName: string | null })
                            .name
                        }
                      </a>
                    ) : (
                  ...
                  <td className="py-4 px-4 whitespace-nowrap text-sm text-gray-500 max-w-xs truncate">
                    {activeTab === "all"
                      ? (task as Task & { creatorFullName: string | null })
                          .creatorFullName
                      : (
                          task as AssingedTask & { taskName: string | null } & {
                            authorName: string | null;
                          }
                        ).authorName}
                  </td>
